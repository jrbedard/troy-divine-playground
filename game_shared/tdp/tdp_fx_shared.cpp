	...
	)
{
...
#ifdef CLIENT_DLL
	C_TDPPlayer *pPlayer = ToTDPPlayer( ClientEntityList().GetBaseEntity( iPlayerIndex ) );
#else
	CTDPPlayer *pPlayer = ToTDPPlayer( UTIL_PlayerByIndex( iPlayerIndex) );
#endif
	...
	CTDPWeaponInfo *pWeaponInfo = static_cast< CTDPWeaponInfo* >( GetFileWeaponInfoFromHandle( hWpnInfo ) );
