...
// ------------------------------------------------------------------------------------------------ //
// CTDPPlayerAnimState declaration.
// ------------------------------------------------------------------------------------------------ //

class CTDPPlayerAnimState : public CBasePlayerAnimState, public ITDPPlayerAnimState
{
public:
	DECLARE_CLASS( CTDPPlayerAnimState, CBasePlayerAnimState );
	friend ITDPPlayerAnimState* CreatePlayerAnimState( CBaseAnimatingOverlay *pEntity, ITDPPlayerAnimStateHelpers *pHelpers, LegAnimType_t legAnimType, bool bUseAimSequences );
	...
	void InitTDP( CBaseAnimatingOverlay *pPlayer, ITDPPlayerAnimStateHelpers *pHelpers, LegAnimType_t legAnimType, bool bUseAimSequences );
...
private:
	...
	ITDPPlayerAnimStateHelpers *m_pHelpers;
};
...
{
	CTDPPlayerAnimState *pRet = new CTDPPlayerAnimState;
	pRet->InitTDP( pEntity, pHelpers, legAnimType, bUseAimSequences );
	return pRet;
}

// ------------------------------------------------------------------------------------------------ //
// CTDPPlayerAnimState implementation.
// ------------------------------------------------------------------------------------------------ //

CTDPPlayerAnimState::CTDPPlayerAnimState()
{
	m_pOuter = NULL;
	m_bReloading = false;
}
...
void CTDPPlayerAnimState::InitTDP( CBaseAnimatingOverlay *pEntity, ITDPPlayerAnimStateHelpers *pHelpers, LegAnimType_t legAnimType, bool bUseAimSequences )
{
...
void CTDPPlayerAnimState::ClearAnimationState()
{
	m_bJumping = false;
	m_bFiring = false;
	m_bReloading = false;
...
void CTDPPlayerAnimState::DoAnimationEvent( PlayerAnimEvent_t event )
{
	...
	{
		Assert( !"CTDPPlayerAnimState::DoAnimationEvent" );
	}
}


float g_flThrowGrenadeFraction = 0.25;
bool CTDPPlayerAnimState::IsThrowingGrenade()
{
...
int CTDPPlayerAnimState::CalcReloadLayerSequence()
{
	const char *pSuffix = GetWeaponSuffix();
	if ( !pSuffix )
		return -1;

	CTDPWeaponBase *pWeapon = m_pHelpers->TDPAnim_GetActiveWeapon();
	if ( !pWeapon )
		return -1;
...
/*
	...
	if ( pWeapon->GetTDPWpnData().m_WeaponType == WEAPONTYPE_PISTOL )
	{
...
#ifdef CLIENT_DLL
	void CTDPPlayerAnimState::UpdateLayerSequenceGeneric( int iLayer, bool &bEnabled, float &flCurCycle, int &iSequence, bool bWaitAtEnd )
	{
		if ( !bEnabled )
			return;
...
bool CTDPPlayerAnimState::IsOuterGrenadePrimed()
{
	CBaseCombatCharacter *pChar = m_pOuter->MyCombatCharacterPointer();
	if ( pChar )
	{
		//CBaseTDPGrenade *pGren = dynamic_cast<CBaseTDPGrenade*>( pChar->GetActiveWeapon() );
...
void CTDPPlayerAnimState::ComputeGrenadeSequence()
{
#ifdef CLIENT_DLL
	if ( m_bThrowingGrenade )
	{
...
int CTDPPlayerAnimState::CalcGrenadePrimeSequence()
{
	return CalcSequenceIndex( "idle_shoot_gren1" );
}


int CTDPPlayerAnimState::CalcGrenadeThrowSequence()
{
	return CalcSequenceIndex( "idle_shoot_gren2" );
}


int CTDPPlayerAnimState::GetOuterGrenadeThrowCounter()
{
	CTDPPlayer *pPlayer = dynamic_cast<CTDPPlayer*>( m_pOuter );
	if ( pPlayer )
...
void CTDPPlayerAnimState::ComputeReloadSequence()
{
#ifdef CLIENT_DLL
...
int CTDPPlayerAnimState::CalcAimLayerSequence( float *flCycle, float *flAimSequenceWeight, bool bForceIdle )
{
	const char *pSuffix = GetWeaponSuffix();
	if ( !pSuffix )
		return 0;
...
const char* CTDPPlayerAnimState::GetWeaponSuffix()
{
	...
	const char *pSuffix = pWeapon->GetTDPWpnData().m_szAnimExtension;

	return pSuffix;
}


int CTDPPlayerAnimState::CalcFireLayerSequence(PlayerAnimEvent_t event)
{
	// Figure out the weapon suffix.
	CTDPWeaponBase *pWeapon = m_pHelpers->TDPAnim_GetActiveWeapon();
	if ( !pWeapon )
...
bool CTDPPlayerAnimState::CanThePlayerMove()
{
	return m_pHelpers->TDPAnim_CanMove();
}


float CTDPPlayerAnimState::GetCurrentMaxGroundSpeed()
{
...
bool CTDPPlayerAnimState::HandleJumping()
{
	if ( m_bJumping )
	{
...
Activity CTDPPlayerAnimState::CalcMainActivity()
{
...
void CTDPPlayerAnimState::DebugShowAnimState( int iStartLine )
{
#ifdef CLIENT_DLL
...
void CTDPPlayerAnimState::ComputeSequences()
{
...
void CTDPPlayerAnimState::ClearAnimationLayers()
{
	if ( !m_pOuter )
		return;
...
void CTDPPlayerAnimState::ComputeFireSequence()
{
#ifdef CLIENT_DLL
