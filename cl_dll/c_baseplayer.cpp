...
CBaseEntity	*C_BasePlayer::GetObserverTarget() const	// returns players targer or NULL
{
	...
	{
		return HLTVCamera()->GetObserverTraget();
	}
	if ( GetObserverMode() == OBS_MODE_ROAMING )
	{
		return NULL;	// no traget in roaming mode
	}
	else
	{
...
CBaseEntity	*C_BasePlayer::GetObserverTargetPlayer() const
{
	return GetObserverTarget();
}
...
void C_BasePlayer::OnRestore()
{
	...
	if ( IsLocalPlayer() )
	{
		...
		//TDP_TODO Do we really want to debounce the third attack button ?
...
void C_BasePlayer::DetermineVguiInputMode( CUserCmd *pCmd )
{
	...
	if (m_pCurrentVguiScreen.Get() && (pCmd->buttons & (IN_ATTACK | IN_ATTACK2 | IN_ATTACK3)) )
	{
		SetVGuiScreenButtonState( m_pCurrentVguiScreen.Get(), pCmd->buttons );

		// Kill all attack inputs if we're in vgui screen mode
		pCmd->buttons &= ~(IN_ATTACK | IN_ATTACK2 | IN_ATTACK3);
		return;
	}
	...
	if ( ((pCmd->buttons & IN_ATTACK) || (pCmd->buttons & IN_ATTACK2) || (pCmd->buttons & IN_ATTACK3)) && !m_pCurrentVguiScreen.Get() )
	{
		bAttacking = true;
	}
...
void C_BasePlayer::GetPredictionErrorSmoothingVector( Vector &vOffset )
{
	...
	float errorAmount = RemapVal(
		gpGlobals->curtime, 
		m_flPredictionErrorTime,
		m_flPredictionErrorTime + cl_smoothtime.GetFloat(),
		1,
		0 );
		
	if ( errorAmount >= 0 && errorAmount <= 1 )
	{
