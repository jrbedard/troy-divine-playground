...
ConVar r_RamViewBlendTo( "r_RamViewBlendTo", "1", FCVAR_CHEAT );
ConVar r_RamViewBlendToScale( "r_RamViewBlendToScale", "0.03", FCVAR_CHEAT );
ConVar r_RamViewBlendToTime( "r_RamViewBlendToTime", "1.5", FCVAR_CHEAT );
ConVar r_RamFOV( "r_RamFOV", "90", FCVAR_CHEAT );

#define RAM_DELTA_LENGTH_MAX	12.0f			// 1 foot
#define RAM_FRAMETIME_MIN		1e-6
#define RAM_HEADLIGHT_DISTANCE 1000

//=============================================================================
//
// Client-side Ram Class
//
class C_PropRam : public C_PropVehicleDriveable
{

	DECLARE_CLASS( C_PropRam, C_PropVehicleDriveable );
...
IMPLEMENT_CLIENTCLASS_DT( C_PropRam, DT_PropRam, CPropRam )
...
C_PropRam::C_PropRam()
{
	...
	//m_pHeadlight = NULL;
	m_ViewSmoothingData.flFOV = r_RamFOV.GetFloat();
}
...
void C_PropRam::Simulate( void )
{
	...
	if ( m_bHeadlightIsOn )
	{
		...
		if ( iAttachment != -1 )
		{
			...
			m_pHeadlight->UpdateLight( vVector, vecForward, vecRight, vecUp, RAM_HEADLIGHT_DISTANCE );
		}
	}
...
//-----------------------------------------------------------------------------
void C_PropRam::UpdateViewAngles( C_BasePlayer *pLocalPlayer, CUserCmd *pCmd )
{
	if ( r_RamViewBlendTo.GetInt() )
	{
		...
		if ( m_flViewAngleDeltaTime > r_RamViewBlendToTime.GetFloat() )
		{
			...
			QAngle outAngles;
			InterpolateAngles( pCmd->viewangles, vehicleEyeAngles, outAngles, r_RamViewBlendToScale.GetFloat() );
...
// Purpose:
//-----------------------------------------------------------------------------
void C_PropRam::DampenEyePosition( Vector &vecVehicleEyePos, QAngle &vecVehicleEyeAngles )
{
...
//-----------------------------------------------------------------------------
void C_PropRam::ComputePDControllerCoefficients( float *pCoefficientsOut,
...
// Purpose:
//-----------------------------------------------------------------------------
void C_PropRam::DampenForwardMotion( Vector &vecVehicleEyePos, QAngle &vecVehicleEyeAngles, float flFrameTime )
{
	...
	if ( flDeltaLength > RAM_DELTA_LENGTH_MAX )
	{
		// Clamp.
		float flDelta = flDeltaLength - RAM_DELTA_LENGTH_MAX;
		if ( flDelta > 40.0f )
		{
		...
		{
			// Position clamp.
			float flRatio = RAM_DELTA_LENGTH_MAX / flDeltaLength;
			vecDeltaPos *= flRatio;
...
// Purpose:
//-----------------------------------------------------------------------------
void C_PropRam::DampenUpMotion( Vector &vecVehicleEyePos, QAngle &vecVehicleEyeAngles, float flFrameTime )
{
	// Get up vector.
	Vector vecUp;
	AngleVectors( vecVehicleEyeAngles, NULL, NULL, &vecUp );
	vecUp.z = clamp( vecUp.z, 0.0f, vecUp.z );
	vecVehicleEyePos.z += r_RamViewZHeight.GetFloat() * vecUp.z;
...
// Purpose: 
//-----------------------------------------------------------------------------
void C_PropRam::OnEnteredVehicle( C_BasePlayer *pPlayer )
{
	int eyeAttachmentIndex = LookupAttachment( "vehicle_driver_eyes" );
	Vector vehicleEyeOrigin;
	QAngle vehicleEyeAngles;
...
// Input  : &data - 
//-----------------------------------------------------------------------------
void WheelDustCallback2( const CEffectData &data )
{
...
DECLARE_CLIENT_EFFECT( "WheelDust2", WheelDustCallback2 );