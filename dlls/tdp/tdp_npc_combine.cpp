...
LINK_ENTITY_TO_CLASS( tdp_npc_combine, CTDP_NPC_Combine );

//---------------------------------------------------------
// Save/Restore
//---------------------------------------------------------
BEGIN_DATADESC( CTDP_NPC_Combine )
...
// Constructor.
//------------------------------------------------------------------------------
CTDP_NPC_Combine::CTDP_NPC_Combine()
{
...
// Create components
//-----------------------------------------------------------------------------
bool CTDP_NPC_Combine::CreateComponents()
{
...
//------------------------------------------------------------------------------
void CTDP_NPC_Combine::InputLookOff( inputdata_t &inputdata )
{
...
//------------------------------------------------------------------------------
void CTDP_NPC_Combine::InputLookOn( inputdata_t &inputdata )
{
	m_spawnflags &= ~SF_COMBINE_NO_LOOK;
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTDP_NPC_Combine::InputStartPatrolling( inputdata_t &inputdata )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTDP_NPC_Combine::InputStopPatrolling( inputdata_t &inputdata )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTDP_NPC_Combine::InputAssault( inputdata_t &inputdata )
{
...
//-----------------------------------------------------------------------------
void CTDP_NPC_Combine::InputHitByBugbait( inputdata_t &inputdata )
{
...
// Purpose:
//-----------------------------------------------------------------------------
void CTDP_NPC_Combine::Precache()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CTDP_NPC_Combine::Activate()
{
...
//
//-----------------------------------------------------------------------------
void CTDP_NPC_Combine::Spawn( void )
{
...
//-----------------------------------------------------------------------------
bool CTDP_NPC_Combine::CreateBehaviors()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CTDP_NPC_Combine::PostNPCInit()
{
	if( IsElite() )
	{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTDP_NPC_Combine::PrescheduleThink()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CTDP_NPC_Combine::DelayAltFireAttack( float flDelay )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CTDP_NPC_Combine::DelaySquadAltFireAttack( float flDelay )
{
	...
	while ( pSquadmate )
	{
		CTDP_NPC_Combine *pCombine = dynamic_cast<CTDP_NPC_Combine*>(pSquadmate);
...
//-----------------------------------------------------------------------------
float CTDP_NPC_Combine::MaxYawSpeed( void )
{
	switch( GetActivity() )
	{
	case ACT_TURN_LEFT:
...
// Output :
//-----------------------------------------------------------------------------
bool CTDP_NPC_Combine::OverrideMoveFacing( const AILocalMoveGoal_t &move, float flInterval )
{
...
//
//-----------------------------------------------------------------------------
Class_T	CTDP_NPC_Combine::Classify ( void )
{
...
//-----------------------------------------------------------------------------
bool CTDP_NPC_Combine::IsCurTaskContinuousMove()
{
...
//-----------------------------------------------------------------------------
void CTDP_NPC_Combine::StartTaskChaseEnemyContinuously( const Task_t *pTask )
{
...
void CTDP_NPC_Combine::RunTaskChaseEnemyContinuously( const Task_t *pTask )
{
	if (!GetNavigator()->IsGoalActive())
	{
		SetIdealActivity( GetStoppedActivity() );
...
// start task
//=========================================================
void CTDP_NPC_Combine::StartTask( const Task_t *pTask )
{
...
// RunTask
//=========================================================
void CTDP_NPC_Combine::RunTask( const Task_t *pTask )
{
	/*
	{
		CBaseEntity *pEnemy = GetEnemy();
...
// Output  :
//------------------------------------------------------------------------------
Vector CTDP_NPC_Combine::BodyTarget( const Vector &posSrc, bool bNoisy ) 
{
...
// Purpose:
//------------------------------------------------------------------------------
bool CTDP_NPC_Combine::FVisible( CBaseEntity *pEntity, int traceMask, CBaseEntity **ppBlocker )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CTDP_NPC_Combine::Event_Killed( const CTakeDamageInfo &info )
{
...
//-----------------------------------------------------------------------------
bool CTDP_NPC_Combine::UpdateEnemyMemory( CBaseEntity *pEnemy, const Vector &position, CBaseEntity *pInformer )
{
	if( m_spawnflags & SF_COMBINE_NO_LOOK )
	{
		return false;
...
//-----------------------------------------------------------------------------
void CTDP_NPC_Combine::BuildScheduleTestBits( void )
{
...
//-----------------------------------------------------------------------------
Activity CTDP_NPC_Combine::NPC_TranslateActivity( Activity eNewActivity )
{
...
// Output :
//-----------------------------------------------------------------------------
int CTDP_NPC_Combine::GetSoundInterests( void )
{
...
//-----------------------------------------------------------------------------
bool CTDP_NPC_Combine::QueryHearSound( CSound *pSound )
{
...
// Output :
//-----------------------------------------------------------------------------
void CTDP_NPC_Combine::AnnounceAssault(void)
{
...
void CTDP_NPC_Combine::AnnounceEnemyType( CBaseEntity *pEnemy )
{
...
void CTDP_NPC_Combine::AnnounceEnemyKill( CBaseEntity *pEnemy )
{
	if (!pEnemy )
		return;
...
//-----------------------------------------------------------------------------
int CTDP_NPC_Combine::SelectCombatSchedule()
{
	// -----------
	// dead enemy
	// -----------
...
// Output :
//-----------------------------------------------------------------------------
int CTDP_NPC_Combine::SelectSchedule( void )
{
...
//-----------------------------------------------------------------------------
bool CTDP_NPC_Combine::ShouldChargePlayer()
{
...
int CTDP_NPC_Combine::SelectScheduleAttack()
{
	// Drop a grenade?
...
// Output :
//-----------------------------------------------------------------------------
int CTDP_NPC_Combine::TranslateSchedule( int scheduleType ) 
{
	switch( scheduleType )
	{
...
//=========================================================
//=========================================================
void CTDP_NPC_Combine::OnStartSchedule( int scheduleType ) 
{
}
...
//=========================================================
void CTDP_NPC_Combine::HandleAnimEvent( animevent_t *pEvent )
{
	Vector vecShootDir;
...
// Output :
//-----------------------------------------------------------------------------
Vector CTDP_NPC_Combine::Weapon_ShootPosition( )
{
...
// started moving.
//=========================================================
void CTDP_NPC_Combine::SpeakSentence( int sentenceType )
{
	switch( sentenceType )
	{
	case 0: // assault
...
// PainSound
//=========================================================
void CTDP_NPC_Combine::PainSound ( void )
{
...
// Output :
//-----------------------------------------------------------------------------
void CTDP_NPC_Combine::LostEnemySound( void)
{
...
// Output :
//-----------------------------------------------------------------------------
void CTDP_NPC_Combine::FoundEnemySound( void)
{
...
void CTDP_NPC_Combine::AlertSound( void)
{
...
// NotifyDeadFriend
//=========================================================
void CTDP_NPC_Combine::NotifyDeadFriend ( CBaseEntity* pFriend )
{
...
// DeathSound 
//=========================================================
void CTDP_NPC_Combine::DeathSound ( void )
{
	// NOTE: The response system deals with this at the moment
	if ( GetFlags() & FL_DISSOLVING )
		return;
...
// IdleSound 
//=========================================================
void CTDP_NPC_Combine::IdleSound( void )
{
...
// Output :
//-----------------------------------------------------------------------------
int	CTDP_NPC_Combine::RangeAttack2Conditions( float flDot, float flDist ) 
{
	return COND_NONE;
}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CTDP_NPC_Combine::CanThrowGrenade( const Vector &vecTarget )
{
	if( m_iNumGrenades < 1 )
	{
		// Out of grenades!
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CTDP_NPC_Combine::CanAltFireEnemy( bool bUseFreeKnowledge )
{
	if (!IsElite() )
		return false;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CTDP_NPC_Combine::CanGrenadeEnemy( bool bUseFreeKnowledge )
{
	if( IsElite() )
		return false;
...
// Output :
//-----------------------------------------------------------------------------
int CTDP_NPC_Combine::MeleeAttack1Conditions ( float flDot, float flDist )
{
	if (flDist > 64)
	{
	...
	if ( dynamic_cast<CTDPHeadcrab *>(GetEnemy()) != NULL )
	{
		return COND_NONE;
	}
...
// Output : Vector
//-----------------------------------------------------------------------------
Vector CTDP_NPC_Combine::EyePosition( void ) 
{
	if ( m_bStanding )
	{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
Vector CTDP_NPC_Combine::GetAltFireTarget()
{
	Assert( IsElite() );
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CTDP_NPC_Combine::IsCrouchedActivity( Activity activity )
{
...
// Output : Vector
//-----------------------------------------------------------------------------
Vector CTDP_NPC_Combine::EyeOffset( Activity nActivity )
{
	if (CapabilitiesGet() & bits_CAP_DUCK)
	{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CTDP_NPC_Combine::SetActivity( Activity NewActivity )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
NPC_STATE CTDP_NPC_Combine::SelectIdealState( void )
{
	switch ( m_NPCState )
	{
		case NPC_STATE_COMBAT:
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CTDP_NPC_Combine::OnBeginMoveAndShoot()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CTDP_NPC_Combine::OnEndMoveAndShoot()
{
	VacateStrategySlot();
}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
WeaponProficiency_t CTDP_NPC_Combine::CalcWeaponProficiency( CBaseCombatWeapon *pWeapon )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CTDP_NPC_Combine::HasShotgun()
{
...
//-----------------------------------------------------------------------------
bool CTDP_NPC_Combine::HandleInteraction(int interactionType, void *data, CBaseCombatCharacter *sourceEnt)
{
...
AI_BEGIN_CUSTOM_NPC( tdp_npc_combine, CTDP_NPC_Combine )
